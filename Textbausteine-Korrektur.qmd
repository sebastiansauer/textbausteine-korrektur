...
## FORMALIA (Checkliste Seminararbeit, xxcheckformalia, xxcf)
...
3. Stellt die Einleitung die Relevanz und die [Forschungsfrage](#fofrage) bzw. das Ziel der Arbeit dar?
...
# CHECKLISTE -- REVIEW (xxreview)


## Methoden

- Wurden Ein- bzw. Ausschlusskriterien definiert?
- Wurde ggf. ein PRISMA-Diagramm verwendet oder die genauen Stichprobenzahlen anderweitig dokumentiert?
- Wurden die Suchbegriffe und Datenbanken dokumentiert?


## Ergebnisse

- Wurden die Ergebnisse nicht nur aufgelistet, sondern in ihrer Relevanz theoriebildend zusammengefasst?
...
- NEG Die Reihenfolge der Kapitel im Inhaltsverzeichnis ist Peters und Dörfler (2019) zu entnehmen: Titelblatt, Abstrsct, Inhaltsverzeichnis, Abbildungsverzeichnis (optional), Tabellenverzeichnis (optional), Abkürzungsverzeichnis (optional), Einleitung, Theorie, Methode, Ergebnisse, Diskussion, Literaturverzeichnis (nicht nummeriert), ggf. Anhänge, ggf. eidesstattliche Erklärung
...
## Titel und Titelseite (xxtitelseite)
...
- NEG Auf der Titelseite ist der *Name* der *Autorin* bzw. des Autors *wichtiger* als der Name der *Gutachterin* bzw. des Gutachters. Daher sollte Ersteres größer als Letzteres gesetzt sein. Dieser Punkt ist in der vorliegenden Arbeit nicht (vollständig) umgesetzt.
...
- NEG *"Meta-Aussagen"* sind (im Abstract und auch andernorts) zu vermeiden: Generische Beispiele für solche Meta-Aussagen, die zwar nicht unbedingt im gleichen Wortlaut in der vorliegenden Arbeit zu finden, aber dennoch in vergleichbarer Form in der vorliegenden Arbeit zu finden sind, sind im Folgenden aufgeführt. Diese Beispiele sollen helfen, den Wesenskern solcher Meta-Aussagen besser zu verstehen und auf dieser Basis künftig präziser zu formulieren. "Es wird das Design begründet", "Es werden die Hypothesen vorgestellt", "Implikationen werden diskutiert", etc. Besser ist es, die Inhalte dieser Aussagen anzugeben, etwa "Die zentrale Hypothese lautet, dass X zu Y führt" oder "Es handelt sich um ein kontrolliertes, randomisiertes Experiment", "Im Anschluss wurden die Daten analysiert", "Eine Limitation dieser Studie ist X". Zugespitzt formuliert: Aussagen der Art "Diese Studie hat eine Hypothese" sind *inhaltsarm* und daher in einer wissenschaftlichen Arbeit fehl am Platz. In einer wissenschaftlichen Arbeit sollten Sie stets auf präzise, inhaltsdichte Formulierungen achten. Beispiele für solche präziseren Aussagen sind: "Diese Arbeit beruft sich auf Theorie XYZ, die besagt ABC", "Frauen parkten im Schnitt 10 Sekunden schneller ein als Männer", "Die zentrale Hypothese besagt, dass Lernen die Klausurnote erhöht". *Ein konkretes Beispiel* für solche Meta-Aussagen in der vorliegenden Arbeit ist: "Basierend auf den Ergebnissen wurden Implikationen abgeleitet, anhand derer die Arbeitszufriedenheit weiterhin gesteigert werden kann" (Abstract). 
...
- NEG Laut APA7 ist der *Abstract nach der Titelseite und vor der Gliederung* zu platzieren. Dieser Punkt wurde in der vorliegenden Arbeit nicht umgesetzt.

- NEG Der Abstract ist relativ *lang*.
...
- NEG Der *Gender-Hinweis* benötigt (und verdient) *keine eigene Seite*.
...
- POS Die *Seitenränder sind nicht zu klein (also passend) gewählt*. Größere Seitenränder wirken optisch ansprechender; lange Textzeilen hingegen sind weniger gut lesbar. Allgemein wird von einer optimalen Lesbarkeit von etwa 66 Zeichen pro Zeile ausgegangen.
...
- NEG Alle Seiten einer entsprechenden Arbeit sollten nummeriert sein. Dieser Punkt ist in der vorliegenden Arbeit nicht komplett umgesetzt.
...
- NEG Absätze sollten *länger als ein Satz sein*; in der vorliegenden Arbeit gibt es einen oder mehrere Absätze, die nur aus einem Satz bestehen, was typografisch nicht wünschenswert ist. Vermeiden Sie solch kurze *Absatzlängen*.
...
- NEG Der *Absatzwechsel* sollte *einheitlich gestaltet* sein, also konsistent eine einzige Methode verwenden.

- NEG Ein *Zeilenumbruch* leitet *keinen* Absatzwechsel ein.
...
## Wortzahl/Umfang (xxwortzahl)

Der Gutachter beziffert seine Anforderung an das grobe *Mengengerüst* im Hinblick auf die Seiten- bzw. Wortzahl einer *Abschlussarbeit* wie folgt: 

1. Bachelor: ca. *12000 Wörter* (40-60 Seiten)

2. Master: ca. *18000 Wörter* (ca. 60-80 Seiten).
...
- NEG Die vorliegende Arbeit umfasst ca. 10000 Wörter; 10/12 entspricht ca. 83%. Damit liegt die Arbeit *knapp über der Länge*, die zum Bestehen in den formalen Vorgaben angegeben wird. 
...
- POS Es wurde korrekt gekoppelt, d.h. die Zusammenschreibung von Wörtern war korrekt (z.B. "Human-Robot-Interaktion").
...
- NEG *Software solte auch zitiert* werden, dazu zählen auch R-Pakete.
...
- NEG Ein (technischer) Bericht sollte, laut APA (s. Kap. 10.4, APA7) die DOI oder die URL enthalten. In der vorliegenden Arbeit ist dies nicht durchgehend der Fall.
...
- NEG Diagramme oder Tabellen sollten *genug Informationen bzw. Erklärung enthalten, um für sich selber verständlich zu sein* (d.h. ohne den Kontext des Haupttextes). So sollte etwa der Titel eines Diagramms oder einer Tabelle die Hauptaussage der Tabelle beschreiben (nicht interpretieren).
...
- POS Das Design ist so gewählt, dass es eine empirische Analyse der Forschungsfrage sinnvoll erlaubt.



`--`

- POS Das Design ist nicht so gewählt, dass es eine empirische Analyse der Forschungsfrage sinnvoll erlauben würde.
...
s. [Hinweisbuch für quantitative Projektarbeit](https://hinweisbuch.netlify.app/hinweise-pruefung-projektarbeit-frame)
...
- NEG In *Spalten, die Werte (Zahlen) enthalten sind, sollten diese Werte zentriert* sein.
...
### Daten-Diagramme, Abbildungen (xxdataviz) {#datadiagrams}
...
- NEG Alle Teile eines Diagramms sollten (ohne Vergrößerung) gut lesbar sein; die Schrift sollte groß genug sein und ohne Serifen (vgl. APA V/ , Kap. 7.2, S. 196).
...
- POS Es wurde eine *inferenzstatistische Methode verwendet*, z.B. mittels des p-Werts.
...
- POS Insgesamt wurde eine inferenzstatqistische Analyse mit den Methoden der *Bayes*-Methode *sinnvoll* durchgeführt.
...
- NEG Das Bayes-Modell wurde *nicht spezifiziert* (im Hinblick auf Prior-Verteilungen und ihre Parameter bzw. Likelihood-Form).
...
- NEG Die APA empfiehlt *möglichst* *wenig Dezimalstellen* zu verwenden, bzw. nur so viele wie nötig (und so wenige wie möglich), ceteris paribus.
...
- NEG Bei *Prozentzahlen* sollte man in der Regel *auf Nachkommastellen verzichten* (vgl. APA Publication Manul, Kapitel 6.36, S. 180), was hier nicht getan wurde.
...
- NEG Bitte lesen Sie in den *APA-Hinweisen* nach zur Verwendung von Dezimalstellen (z.B. [hier](https://apastyle.apa.org/instructional-aids/numbers-statistics-guide.pdf)) .
...
### R-/Python-Kompetenz (xxrkomp xxrcomp)
...
- NEG Es wurden *weniger gut lesbarer Code* für Verfahren der Datenaufbereitung verwendet. So ist dieses Methode weniger gut lesbar: `difwt.frauen <- (Report$repwt[Report$sex == "F"])-(Report$weight[Report$sex == "F"])`. Besser lesbar wäre: ` Report %>% mutate(difference_weight = repwt - weight)`.
...
- NEG Es *fehlen* die *R-Ausgaben* im Bericht.
...
### Auswahl der qualitativen Methode


`+++`

- POS Im Rahmen der qualitativen Analyse wurde eine große Menge an Codes/Kategorien vergeben.

- POS Das Vorgehen im Rahmen der qualitativen Analyse mit Hilfe der Grounded Theory ist wohl begründet.
...
- NEG Die unique Stärke und partikuläre Eigenart der Grounded Theory ist ihre Fähigkeit zur Theoriebildung. Dieser Stärke wurde in der vorliegenden Arbeit nicht in einem vollem Umfang Rechnung getragen.
...
### Kategorien(-system) {#kategoriensystem}
...
- POS Im Rahmen der qualitativen Aanalyse wurde eine *große Menge an Codes/Kategorien* vergeben.
...
## Systematische Review (xxsystrev)
...
- POS Der *Umfang* an (in der Volltext-Analyse) analysierten Primärquellen ist *umfassend*, n=??.

- POS Der *Umfang* (in der Volltext-Analyse)an analysierten Primärquellen ist *angemessen*, n=??.

- POS Ein- und/oder *Ausschlusskriterien* wurden berichtet.

- POS Die verwendeten Ein- bzw. *Ausschlusskriterien* sind *gegenstandsangemessen.*
...
- POS Die Methodik der systematischen Literaturanalyse ist stimmig vorgestellt. 
...
- NEG Der *Publikationszeitraum* für (einzuschließende) Studien sollte angegeben werden, fehlt aber in der vorliegenden Arbeit (zumindest an der dafür geeigneten Stelle).

- NEG Die Ein- bzw. Ausschlusskriterien für Studien ist genau zu definieren. Dieser Punkt ist in der vorliegenden Arbeit nicht ausreichend berücksichtigt.


`000`

- NEUTRAL Die Arbeit könnte noch an *Profilschärfe* gewinnen, wenn die *zentralen Befunde in theoriebildender (kreativer, überraschender) Form verknüpft dargestellt* werden und weniger der Reihe nach (nur) aufgezählt werden. 
...
## Forschungsfrage, Problemstellung, Relevanz, Hintergrund und Zielsetzung (xxfofrage) {#fofrage}
...
- POS Die *Relevanz* der Forschungsfrage wurde klar herausgestellt.
- POS Der *Hintergrund* der Arbeit wurde klar herausgestellt.
...
- NEG Die *Darstellung grundlegender psychologischer Inhalte* (z.B. Big Five) sollte in einer psychologischen Abschlussarbeit, die entsprechendes Fachpublikum voraussetzt, eher *knapper* gehalten sein, als es in der vorliegenden Arbeit gehalten wird.
...
- NEG Der Praxisorientierung der Arbeit wurde -- an einigen Stellen, aber keineswegs durchgehend -- wissenschaftliche Qualität geopfert. Dabei sind beide Ansprüche -- Praxis und Wissenschaft -- keineswegs im Widerspruch zueinander. Denn Wissenschaft meint hier nichts anderes, als ein hoher Qualitätsanspruch in der Stichhaltigkeit der Ergebnisse. Kein Praktiker wird diesem Kriterium widersprechen wollen. Man kann sogar soweit gehen, zu behaupten, dass die Stichhaltigkeit der Ergebnisse, die Wissenschaftlichkeit also, die notwendige Bedingung für Praxisrelevanz darstellt. Schließlich mag ein Ergebnis so praxisrelevant sein wie es will, wenn es nicht tatsachenkonform ist, spielt die Praxisrelevanz keine Rolle. Diese Ausführung ist als Plädoyer gedacht, bei Arbeiten, die mit durchaus laudablen Anspruch an Praxisrelevanz antreten, die Wissenschaftlichkeit nicht opfern müssen, im Gegenteil.
...
- NEG Aufgabe des Kapitels *Einleitung* ist es (nur), die Forschungsfrage anzuführen, zu detaillieren und zu motivieren bzw. (in ihrer Relevanz) zu begründen (in dieser Prioritätenreihenfolge); ggf. darf sich ein Überblick über die Arbeit anschließen. Sonstige Inhalte sollten sich nicht in der Einleitung finden. Die vorliegende Arbeit erfüllt diese Anforderung nicht vollständig.
...
- NEG Der *Theorieteil* einer entsprechenden Arbeit sollte *nah an der Forschungsfrage ausgerichtet* sein. Dies beinhaltet vor allem die Theorien, die den Forschungsfragen oder Hypothesen zugrunde liegen. Weiterhin bietet es sich an, relevante Konzepte zu definieren. Auf *andere Erläuterungen*, die weiter weg führen, sollte *verzichtet* werden oder zumindest nur kurz eingegangen werden.
...
- NEG *Adjektive* sollten in technischen Berichten bzw. wissenschaftlichen Texten sparsam (nicht ohne Not) verwendet werden. In der vorliegenden Arbeit gibt es Textstellen, in denen dieser Punkt nicht berücksichtigt ist.
...
### Besonderheiten der Video-Präsentation
